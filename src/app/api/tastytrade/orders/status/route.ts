/**
 * Tastytrade Orders API Route
 * Fetches recent and live orders for a given account
 */
...
export async function GET(request: Request) {
    try {
        ...
        const endpoint = liveOnly ? '/orders/live' : '/orders';
        const apiUrl = `${TASTYTRADE_CONFIG.apiBaseUrl}/accounts/${accountNumber}${endpoint}`;
        ...
        let response = await fetchOrders(tokens.accessToken);

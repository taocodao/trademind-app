    ...
    await query(
        `INSERT INTO user_settings (
         ...
         ON CONFLICT (user_id) DO UPDATE SET
            ...
            billing_source    = CASE
                WHEN user_settings.billing_source = 'stripe' AND user_settings.stripe_price_id IS NOT NULL AND user_settings.subscription_status IN ('active', 'trialing', 'past_due') THEN 'stripe'
                ELSE 'whop'
            END,
            subscription_status = CASE
                WHEN user_settings.billing_source = 'stripe' AND user_settings.stripe_price_id IS NOT NULL AND user_settings.subscription_status IN ('active', 'trialing', 'past_due') THEN user_settings.subscription_status
                ELSE EXCLUDED.subscription_status
            END,
            subscription_tier = CASE
                WHEN user_settings.billing_source = 'stripe' AND user_settings.stripe_price_id IS NOT NULL AND user_settings.subscription_status IN ('active', 'trialing', 'past_due') THEN user_settings.subscription_tier
                ELSE $3
            END,
            updated_at = NOW()`,
        [

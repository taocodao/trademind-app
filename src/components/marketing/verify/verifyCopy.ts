...
const EN: VerifyCopy = {
    ...
    recordRows: [
        { name: 'QQQ LEAPS (backtest)', total: '+1,070.7%', cagr: '55.1%', sharpe: '1.83', maxdd: '-14.5%', calmar: '3.79', final: '$292,669' },
        { name: 'QQQ buy & hold', total: '+136.4%', cagr: '16.6%', sharpe: '0.79', maxdd: '-35.6%', calmar: '0.47', final: '$59,089' },
    ],
    recordNote:
        'January 4, 2021 to August 14, 2026, 5.6 continuous years, starting from $25,000. Worst drawdown period: October 11 to October 26, 2023. QQQ buy & hold is measured on the same dates from the same data feed.',
    metricDrillTitle: 'Audit trail for each headline number',
    drillLabels: { formula: 'Formula', inputs: 'Inputs', check: 'Check' },
    metricDrills: [
        {
            key: 'total',
            name: 'Total return: +1,070.7%',
            formula: '(Final NAV - Starting NAV) / Starting NAV = (292,669 - 25,000) / 25,000 = 10.7067.',
            ...
            check: 'Reproduce: sum the P&L column of the public ledger, add residual option marks, divide by 25,000. The public harness run.py prints the same figure.',
        },
        {
            key: 'cagr',
            name: 'CAGR: 55.1%',
            formula: '(Final NAV / Starting NAV) ^ (365.25 / days) - 1, with 2,048 calendar days from 2021-01-04 to 2026-08-14.',
            inputs: 'Same NAV series as total return. The window is fixed and identical for every visitor.',
            check: 'Reproduce: (292,669 / 25,000) ^ (365.25 / 2048) - 1 = 0.551. Printed by run.py in the public harness.',
        },
        {
            key: 'sharpe',
            name: 'Sharpe ratio: 1.83',
        ...
        {
            key: 'calmar',
            name: 'Calmar ratio: 3.79',
            formula: 'CAGR divided by the absolute max drawdown: 0.551 / 0.145 = 3.79.',
    ...
    ledgerLinkDesc: 'Open the interactive ledger: all 1,570 fills marked on the QQQ chart, each expandable to pricing inputs, costs, and entry-gate status.',
    ...
    calendarNote: '2026 is a partial year through August 14. 2022 was the weakest year, barely positive. The record is not smooth, and the table above is not an average of good years.',
    disclaimers: [
        ...
        'Past performance is not necessarily indicative of future results. This is a backtest: no actual account produced these results. The next drawdown could be deeper than -14.5%. TradeMind publishes signals for you to review and enter at your own broker; it never connects to or submits orders to your brokerage.',
    ],
    ...
    auditRows: [
        { check: 'Sharpe above 3 usually means overfitting', result: '1.83', verdict: 'Passes', verdictKind: 'pass' },
        { check: 'Win rate above 90% usually means curve-fitting', result: '85.7% on short calls, 82.5% on SMH spreads, 90.9% on LEAPS entries', verdict: 'Passes, watch the small LEAPS sample', verdictKind: 'pass' },
        { check: 'Zero losing months means the data is wrong', result: '20 losing months out of 67, with 2022 nearly flat at +10.8%', verdict: 'Passes', verdictKind: 'pass' },
        { check: 'Drawdown that never exceeds 10% is cosmetic', result: '-14.5% peak to trough over three weeks in late 2023', verdict: 'Passes', verdictKind: 'pass' },
        { check: 'Returns immune to parameters are real; returns balanced on one setting are not', result: 'One entry threshold set at 0.43 instead of 0.45 adds a single trade and moves CAGR from 54.0% to 55.1%', verdict: 'Disclosed, not hidden', verdictKind: 'warn' },
    ],
    auditNote:
        'The last row is the one to sit with. Two adjacent settings of one gate differ by about a point of CAGR because of a single 2026 entry. We publish both numbers and the ledger entry for that trade. Judge accordingly.',
    ...
    illusRows: [
        ...
        { y: 'Year 5', v: '~$89,400' },
        { y: 'Year 10', v: '~$799,400' },
        { y: 'Year 15', v: '~$7,146,600' },
    ],
    illusVolLabel: 'The path was not smooth',
    illusVol: 'The same record includes a nearly flat 2022 at +10.8% and a -14.5% drawdown in late 2023. Compounding at this rate is an average across those years, not a straight line.',
    ...
    illusFine: '*Hypothetical illustration based on backtested results, not a projection or promise. The rate used is 55% per year, the backtested CAGR rounded down. Backtested performance has many inherent limitations and is not necessarily indicative of future results. A 55% rate sustained for 15 years would exceed nearly all verified long-horizon public track records. Shown for arithmetic illustration only.',
    homeLink: 'New here? Start with the two-minute story on the homepage \u2192',
    backHomeBtn: '\u2190 Back to the homepage',
    ctaLedger: 'Browse all 1,570 fills in the ledger \u2192',
    ...
    kitCards: [
        { title: 'Full trade ledger', desc: 'Every one of the 1,570 fills: timestamp, strike, expiry, price, IV, delta, slippage, commission, P&L, and the rule that triggered it.', href: '/verify/trademind-v4-ledger.csv' },

...
export function ShareModal({
    ...
    const handleShare = useCallback(async () => {
        ...
        const disclosedPost = editedPost;
    ...
    const charCount   = platform === 'twitter'
        ? twitterCharCount(editedPost)
        : (editedPost ? editedPost.length : 0);
        ...
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4">
            ...
            <div className="relative z-10 w-full sm:max-w-2xl bg-[#141420] border border-white/10 rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col max-h-[calc(92dvh-4rem)] sm:max-h-[92dvh] mb-16 sm:mb-0 overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto px-5 pb-5 space-y-4 min-h-0">
                    ...
                    {/* ── Platform selector ── */}
                    <div>
                        <p className="text-[10px] uppercase tracking-wider text-zinc-600 font-semibold mb-2">{t('share_earn.platform', 'Platform')}</p>
                    ...
                    {/* ── Template selector ── */}
                    <div>
                        <p className="text-[10px] uppercase tracking-wider text-zinc-600 font-semibold mb-2">{t('share_earn.template', 'Template')}</p>
                    ...
                    {/* ── Tone selector ── */}
                    <div>
                        <p className="text-[10px] uppercase tracking-wider text-zinc-600 font-semibold mb-2">{t('share_earn.tone', 'Tone')}</p>
                        <div className="flex gap-1.5">
                            ...
                            >
                                ...
                                {isCustomizing ? t('share_earn.customizing', 'Customizing…') : t('share_earn.regenerate', 'Regenerate')}
                            </button>
                        </div>
                    </div>
                    ...
                    {showCustomize && (
                        <div className="flex flex-col gap-2 px-1">
                            <p className="text-xs text-zinc-400">{t('share_earn.what_to_change', 'What would you like to change?')}</p>
                            <textarea
                                autoFocus
                            ...
                            <div className="flex gap-2">
                                ...
                                >
                                    {isCustomizing ? <><RefreshCw className="w-3 h-3 animate-spin" /> {t('share_earn.customizing', 'Customizing…')}</> : <><Sparkles className="w-3 h-3" /> {t('share_earn.apply_changes', 'Apply Changes')}</>}
                                </button>
                                <button
                                ...
                                >{t('share_earn.cancel', 'Cancel')}</button>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="rounded-2xl border border-white/10 bg-black/30 overflow-hidden">
                        {isGenerating ? (
                            <div className="flex flex-col items-center justify-center gap-3 py-12">
                                ...
                                <p className="text-zinc-500 text-sm">{t('share_earn.generating', 'Generating your')} {template} {t('share_earn.post_label', 'post…')}</p>
                            </div>
                        ) : error ? (
                ...
                {!isGenerating && editedPost && (
                    <div className="px-5 pt-3 pb-5 border-t border-white/8 shrink-0 space-y-2.5 bg-[#141420]">
                        ...
                        {(() => {
                            ...
                            return (
                                ...
                                <Share2 className="w-4 h-4" />
                                    {t('share_earn.open_platform', `Open ${cfg.label} · Text Copied to Clipboard`, { label: cfg.label })}
                                </button>
                            );
                        })()}
                        ...
                        {!buildIntentUrl(platform, editedPost, ogCardUrl) && CREATOR_URLS[platform] && (
                            creatorOpened ? (
                                ...
                                >
                                    ...
                                    {t('share_earn.open_creator', `Open ${cfg.label} Creator · Text Copied`, { label: cfg.label })}
                                </button>
                            )
                        )}
                        ...
                        {!buildIntentUrl(platform, editedPost, ogCardUrl) && !CREATOR_URLS[platform] && (
                            ...
                            >
                                {copied
                                    ? <><CheckCircle2 className="w-4 h-4" /> {t('share_earn.copied', 'Copied!')}</>
                                    : <><Copy className="w-4 h-4" /> {t('share_earn.copy_clipboard', 'Copy to Clipboard')}</>
                                }
                            </button>
                        )}

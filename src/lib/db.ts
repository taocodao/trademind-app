...
export async function initializeUserTables(): Promise<void> {
    try {
        ...
        // ── Ambassador Hub: Generated Posts ──────────────────────────────────
        await query(`
            CREATE TABLE IF NOT EXISTS generated_posts (
                id                  SERIAL PRIMARY KEY,
                user_id             VARCHAR(128) REFERENCES user_settings(user_id) ON DELETE SET NULL,
        ...
        `);
        await query(`CREATE INDEX IF NOT EXISTS idx_generated_posts_user     ON generated_posts(user_id)`);
        await query(`CREATE INDEX IF NOT EXISTS idx_generated_posts_platform ON generated_posts(platform)`);
        await query(`CREATE INDEX IF NOT EXISTS idx_generated_posts_created  ON generated_posts(created_at DESC)`);

        // ── Ambassador Hub: Post Library ──────────────────────────────────────
        await query(`
            CREATE TABLE IF NOT EXISTS post_library (
                id                  SERIAL PRIMARY KEY,
                user_id             VARCHAR(128) REFERENCES user_settings(user_id) ON DELETE CASCADE,
        ...
        `);
        await query(`CREATE INDEX IF NOT EXISTS idx_post_library_user     ON post_library(user_id)`);
        await query(`CREATE INDEX IF NOT EXISTS idx_post_library_platform ON post_library(platform)`);
